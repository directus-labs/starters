...
> **Note:** The type generation requires an admin token with permissions to read system collections like `directus_fields`. The public token (`PUBLIC_DIRECTUS_TOKEN`) does not have sufficient permissions for this operation. The script will prompt you to enter the admin token interactively.
