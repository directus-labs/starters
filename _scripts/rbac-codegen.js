...
function copyTemplateTree(src, dest, skipSrc = 'src') {
	if (existsSync(dest)) {
...
function syncI18nPermissions() {
	...
	if (existsSync(i18nDeltaPath)) {
		...
		const licensedKeys = new Set(
			licensedPerms.map((r) => `${r.policy}|${r.collection}|${r.action}`),
		)
...
function checkLicensedTemplate() {
	...
	for (const file of RBAC_FILES) {
		const expected =
			file === 'permissions.json' ? licensedPerms : readJson(join(RBAC, 'core-skeleton', file))
